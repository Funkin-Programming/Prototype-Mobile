...
class PlayState extends FlxTransitionableState
{
			...
			switch (swagCounter)
			{
				...
				case 1:
					...
					FlxTween.tween(ready, {y: ready.y += 100, alpha: 0}, Conductor.crochet / 1000, {
						...
						onComplete: function(_) { ready.destroy(); }
					});
				...
				case 2:
					...
					FlxTween.tween(set, {y: set.y += 100, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(_) { set.destroy(); }
					});
				...
				case 3:
					...
					FlxTween.tween(go, {y: go.y += 100, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(_) { go.destroy(); }
					});
			...
			for (section in noteData)
			{
				...
				for (songNotes in dumbassSection)
				{
					...
					if (songNotes != 0)
					{
						...
						swagNote.x += ((FlxG.width / 2) * playerCounter);
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					{
						case 1: dad.playAnim('singUP');
						case 2: dad.playAnim('singRIGHT');
						case 3: dad.playAnim('singDOWN');
						case 4: dad.playAnim('singLEFT');
					}
	...
	private function keyShit():Void
	{
		...
		if (hitbox != null)
		{
			...
			for (touch in FlxG.touches.list)
			{
				...
				{
					if (touch.overlaps(hitbox.buttonUp))    upP    = true;
					if (touch.overlaps(hitbox.buttonRight)) rightP = true;
					if (touch.overlaps(hitbox.buttonDown))  downP  = true;
					if (touch.overlaps(hitbox.buttonLeft))  leftP  = true;
				}
				if (touch.justReleased)
				{
					if (touch.overlaps(hitbox.buttonUp))    upR    = true;
					if (touch.overlaps(hitbox.buttonRight)) rightR = true;
					if (touch.overlaps(hitbox.buttonDown))  downR  = true;
					if (touch.overlaps(hitbox.buttonLeft))  leftR  = true;
				}
			}
		}
		#end
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
		{
			...
			if (possibleNotes.length > 0)
			{
				for (daNote in possibleNotes)
				{
					...
					{
						case 1: if (upP || rightP || downP || leftP) noteCheck(upP,    daNote);
						case 2: if (upP || rightP || downP || leftP) noteCheck(rightP, daNote);
						case 3: if (upP || rightP || downP || leftP) noteCheck(downP,  daNote);
						case 4: if (upP || rightP || downP || leftP) noteCheck(leftP,  daNote);
					}
				...
				badNoteCheck(upP, rightP, downP, leftP);
		}
		...
		if ((up || right || down || left) && !boyfriend.stunned && generatedMusic)
		{
			notes.forEach(function(daNote:Note)
			{
				if (daNote.canBeHit && daNote.mustPress)
				{
					switch (daNote.noteData)
					{
						case -1: if (up    && daNote.prevNote.wasGoodHit) goodNoteHit(daNote);
						case -2: if (right && daNote.prevNote.wasGoodHit) goodNoteHit(daNote);
						case -3: if (down  && daNote.prevNote.wasGoodHit) goodNoteHit(daNote);
						case -4: if (left  && daNote.prevNote.wasGoodHit) goodNoteHit(daNote);
					}
				}
			});
		}
	...
	function noteMiss(direction:Int = 1):Void
	{
		if (!boyfriend.stunned)
		{
			...
			switch (direction)
			{
				case 1: boyfriend.playAnim('singUPmiss',    true);
				case 2: boyfriend.playAnim('singRIGHTmiss', true);
				case 3: boyfriend.playAnim('singDOWNmiss',  true);
				case 4: boyfriend.playAnim('singLEFTmiss',  true);
			}
		}
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			{
				case 1: boyfriend.playAnim('singUP');
				case 2: boyfriend.playAnim('singRIGHT');
				case 3: boyfriend.playAnim('singDOWN');
				case 4: boyfriend.playAnim('singLEFT');
			}
	...
	private function everyBeat():Void
	{
			...
			|| Conductor.songPosition < lastBeat + Conductor.safeZoneOffset)
		{
			if (Conductor.songPosition > lastBeat + Conductor.crochet)
			{
				...
				if (totalBeats % gfSpeed == 0) gf.dance();
	...
	public static function onWinClose():Void {}
}